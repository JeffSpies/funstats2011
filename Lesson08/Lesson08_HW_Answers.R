...
ddply(x77, c('Region'), summarise, AreaMean=mean(Area), AreaSD=sd(Area))
...
x77$MurdersPerArea <- x77$MurdersTotal/x77$Area
x77sorted <- x77[order(x77$MurdersPerArea, decreasing=T),]
...
ddply(x77, c('Region'), transform, MurdersPerSqMile=mean(MurdersTotal/Area))
...
qplot(year, percent, geom='line', colour=name, 
    data=dat[dat$name=='Calvin' & dat$sex=='boy',])
